...
const MenuItemForm = ({ place, onDone, item = {} }) => {
  ...
  const onAddCategory = async () => {
    ...
    if (json) {
      toast(`Категория ${json.name} создана.`, { type: "success"});
      setCategory(json.id);
      setCategoryName("");
  ...
  const onAddMenuItems = async () => {
    ...
    if (json) {
      toast(`Элемент в меню ${json.name} создан`, { type: "success" });
      setCategory("");
      setName("");
      setPrice(0);
      setDescription("");
  ...
  const onUpdateMenuItem = async () => {
    ...
    if (json) {
      ...
      toast(`Элемент меню ${json.name} обновлен`, { type: "success" });
      setCategory("");
      setName("");
      setPrice(0)
      setDescription("");
